...
public class PropertiesProvider {
	...
	private void loadProperties() throws Exception
	{
		...
		try
		{
			...
			this.table.load(is);
			log.debug("Properties File ["+properties_file+"] was loaded successfully");			
		}catch(Exception e){
			log.fatal("Properties File ["+properties_file+"] was not loaded properly: " + e.getMessage());
			throw e;
		}finally
		{
			try
			...
			{
				log.error("Impossible to close the properties file ["+properties_file+"]:"+e.getMessage());
				return;
			}	
		}
	}
	...
	public Object getValue(String property){
		if(this.table.containsKey(property)){
			...
			log.warn("The property value ["+property+"] asked in the properties file ["+properties_file+"] is missing.");
			return null;
		}
	}
	...
	public void mergeWith(PropertiesProvider pp)
	{
		...
		while(keys.hasMoreElements())
		{
			...
			{
				log.warn("The property ["+elementKey.toString()+"->"+pp.getValue((String)elementKey)+"] already exist in the Property Provider, the old value ["+getValue((String)elementKey)+"] is kept.");
			}
			else
			{
