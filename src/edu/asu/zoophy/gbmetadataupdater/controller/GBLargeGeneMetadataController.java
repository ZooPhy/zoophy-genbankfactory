 ...
 *
 */
public class GBLargeGeneMetadataController implements ControllerInt {
	...
	private final static String INSERT_METADATA = "Update \"Location_Geoname\" "
			...
			+ "Where \"Accession\"=?";
	long numOrigNormalized = 0;
	long numCurNormalized = 0;
	...
	public void run() throws Exception{
		...
		List<Object> queryParams22 = new LinkedList<Object>();
		...
		try {
			...
			query1 = new DBQuery(ResourceProvider.getDBInterfacer(RP_PROVIDED_RESOURCES.ANNOTATION_DBI).getConnection(), DBQuery.QT_SELECT_MULTIPLE_ROWS, this.SELECT_GENEIDs, queryParams1);
			...
			query2 = new DBQuery(ResourceProvider.getDBInterfacer(RP_PROVIDED_RESOURCES.ANNOTATION_DBI).getConnection(), DBQuery.QT_SELECT_MULTIPLE_ROWS, SELECT_GENE, queryParams2);
			//query for updating the geospatial metadata of a record given it's accession number
			query22 = new DBQuery(ResourceProvider.getDBInterfacer(RP_PROVIDED_RESOURCES.ANNOTATION_DBI).getConnection(), DBQuery.QT_SELECT_MULTIPLE_ROWS, SELECT_PRODUCT, queryParams2);
			...
			sb.append("Num originally geocoded:"+this.numOrigNormalized+"\n");
			sb.append("Num geocoded here:"+this.numCurNormalized+"\n");
			sb.append("Printing source count:\n");
			log.info("Num originally geocoded ["+this.numOrigNormalized+"]");
			log.info("Num geocoded here [" + this.numCurNormalized+"]");
