 ...
 *
 */
public class GBLocationTestController implements ControllerInt {
	final private Logger log = Logger.getLogger(GBMetadataUpdater.class);
	private final static String SELECT_ACCESSIONS = "Select \"Accession\" from \"Location_Geoname\" where \"Location\"='unknown'";
	private final static String SELECT_GBMETA = "Select \"Accession\", \"Strain\", \"Isolate\", \"Organism\", \"Definition\" from \"Sequence_Details\" where \"Accession\"=?";
	...
	public void run() throws Exception{
		...
		try {
			...
			query0 = new DBQuery(ResourceProvider.getDBInterfacer(RP_PROVIDED_RESOURCES.ANNOTATION_DBI).getConnection(), DBQuery.QT_SELECT_MULTIPLE_ROWS, SELECT_ACCESSIONS, queryParams0);
			//query for retrieving pertinent record metadata given an accession number
			query1 = new DBQuery(ResourceProvider.getDBInterfacer(RP_PROVIDED_RESOURCES.ANNOTATION_DBI).getConnection(), DBQuery.QT_SELECT_MULTIPLE_ROWS, SELECT_GBMETA, queryParams1);
			...
			while(result0.next()) {
				...
				if(result1.next()) {
					...
					//}
					numProcessed++;
					log.info("Processed record ["+accession+"]");
