...
public class GBTestGeneMetadataController implements ControllerInt {
	...
	private final static String SELECT_COUNT = "Select count(*) as total from \"Sequence_Details\""; // where \"Location\" is null";
	...
	/*private final static String INSERT_METADATA = "Update \"GeneNormalized\" "
			+ "Set \"Gene_Name\"=?, \"Normalized_Gene_Name\"=?, \"Longitude\"=?, \"Type\"=?, \"Country\"=? "
			+ "Where \"Accession\"=?"; */
	...
	public void run() throws Exception {
		...
		ResultSet accessionResult = null;
		ResultSet geneIDResult = null;
...
//		int numProcessed;
		try {
			...
			geneIDQuery = new DBQuery(ResourceProvider.getDBInterfacer(RP_PROVIDED_RESOURCES.ANNOTATION_DBI).getConnection(), DBQuery.QT_SELECT_MULTIPLE_ROWS, this.SELECT_GENEIDs, accessionParams);
			...
			taxIDQuery = new DBQuery(ResourceProvider.getDBInterfacer(RP_PROVIDED_RESOURCES.ANNOTATION_DBI).getConnection(), DBQuery.QT_SELECT_MULTIPLE_ROWS, this.SELECT_TAXID, accessionParams);
				...
				if(id!=null && id.length()>0) {
					...
					for(String gene:genesAdded) {
						if(gene!=null && gene.length()>0) {
							...
							if(taxID!=null && taxID.length()>0) {
								id = glq.searchGeneIDGivenSymbolAndTaxID(name, taxID);
							}
							if(id==null || id.length()==0) {
								String curTaxID = taxID;
								String parentTaxID="";
								while(curTaxID!=null && curTaxID.length()>0) {
									parentTaxID = glq.searchParentGivenTaxID(curTaxID);
									if(parentTaxID.trim().equals("1")) {
										break;
									}
									if(parentTaxID.trim().equals(curTaxID.trim())) {
										break;
									}
									curTaxID = parentTaxID;
									id = glq.searchGeneIDGivenSymbolAndTaxID(name, curTaxID);
									if(id!=null && id.length()>0) {
										break;
									}
								}
							...
							if(id==null || id.length()==0) {
								String parentTaxID = glq.searchParentSpeciesGivenTaxID(taxID);
								Set<String> allTaxIDs =  taxMap.get(parentTaxID);
								if(allTaxIDs==null) {
									allTaxIDs=glq.searchTaxIDsGivenParent(parentTaxID);
									taxMap.put(parentTaxID, allTaxIDs);
								}
								for(String curTaxID: allTaxIDs) {
									id = glq.searchGeneIDGivenSymbolAndTaxID(name, curTaxID);
									if(id!=null && id.length()>0) {
										break;
									}
					...
					for(String product:prodsAdded) {
						...
						if(glq.isGeneSymbol(product)==false||id==null||id.length()==0) {
							Set<String >names = glq.searchSymbolGivenGeneName(product);
							if(names==null || names.size()==0) {
						
								continue;
							}
							for(String gene: names) {
								if(gene.length()==0) {
									continue;
								}
								taxIDResult = taxIDQuery.executeSelect_MultiRows();
								...
								if(taxID!=null && taxID.length()>0) {
									id = glq.searchGeneIDGivenSymbolAndTaxID(gene, taxID);
								}
								if(id==null || id.length()==0) {
									String curTaxID = taxID;
									while(curTaxID!=null && curTaxID.length()>0) {
										String parentTaxID = glq.searchParentGivenTaxID(taxID);
										if(parentTaxID.trim().equals(taxID.trim())){
											break;
										}
										taxID = parentTaxID;
										id = glq.searchGeneIDGivenSymbolAndTaxID(gene, taxID);
