...
public class DBQuery 
{
	...
	public static final int QT_DROP							  			= 10;
		...
		public DBQuery(Connection con, int pQueryType, String query, List<Object> params) throws Exception
		{
			...
			{
				log.error("Impossible to prepare the statement for the query: "+e.getMessage());
				close();
				throw e;
			}catch(Exception e)
			{
				log.error("Other error occured when creating the query to insert:"+e.getMessage());
				close();
				throw e;
			}
			this.params=params;
		...
		public DBQuery(Connection con, int pQueryType, String query) throws Exception
		{
			...
			try 
			{
				...
				{
					log.error("The predifined type of query should be QT_INSERT_BATCH, QT_CREATE_FUNCTION, QT_DROP OR QT_CREATE for this constructor.");
		...
		public void close() 
		{
			...
			{
				log.warn("Impossible to close the ressource for the query ["+pstmt.toString()+"]: "+e.getMessage());
			}
			c = null;
		}
		...
		public void instantiateParams() throws SQLException 
		{
			...
			for (int i=0;i<params.size();i++) 
			{
				try
				{
					...
					} else if (paramClass.equals(java.time.LocalDate.class)) {
				...
				{
					log.error("Parameter's class is unexpected: "+param.getClass().toString());
					close();
					throw new SQLException("Error with the parameter received.");
				}
				}catch(SQLException e)
				{
					log.error("SQL Error when parsing the parameters of the query:"+e.getMessage());
					close();
					throw e;
				}catch(Exception e)
				{
					log.error("Unexpected error during the parsing of the parameters for a query:"+e.getMessage()+", "+e.toString());
					close();
		...
		public void addBatch(List<Object> Params) throws SQLException
		{
			...
			try 
			{
				...
				log.error("SQL error during adding operation in the batch: "+e.getMessage());
				close();
				throw e;
			}
		}
		...
		public void executeBatch() throws SQLException
		{
			...
			{
				log.error("Error(s) occurres when executing the Batch: ");
				...
				log.error("SQL error when executing the batch: "+e.getMessage());
				//c.rollback();
				close();
				throw e;
			}catch(Exception e)
			{
				log.error("Other exceptions occured when executing the SQL batch: "+e.getMessage());
				close();
		...
		public boolean executeCreateFunction() throws SQLException
		{
			try
			{
				...
				log.debug("SQL error during deleting operation:"+e.getMessage());
				//c.rollback();
				throw e;
			}
			finally
		...
		public boolean executeCreate() throws SQLException
		{
			try
			{
				...
				log.error("SQL error during deleting operation:"+e.getMessage());
				//c.rollback();
				throw e;
			}
			finally
		...
		public boolean executeDrop() throws SQLException
		{
			try
			{
				...
				log.error("SQL error during droping operation:"+e.getMessage());
				//c.rollback();
				throw e;
			}
			finally
		...
		public Object executeInsert() throws SQLException
		{
			...
			try
			{
				...
				//the error message should be handle by the object which call the DBQuery
				//c.rollback();
				log.debug("SQL error during inserting operation:"+e.getMessage());
				throw e;
			}
			finally
			{
		...
		public ResultSet executeSelectedRow() throws SQLException
		{
			...
			try 
			{
				...
				log.debug("SQL error during the fetching of the unique row:"+e.getMessage());
				//c.rollback();
				close();
				throw e;
			}
		...
		public ResultSet executeSelect_MultiRows() throws SQLException
		{
			...
			return rs;
				...
				log.error("SQL error during the select query:"+e.getMessage());
				//c.rollback();
				close();
				throw e;
			}
