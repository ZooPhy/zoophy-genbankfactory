...
public class LuceneDocAdder {
	...
	public void index(String indexPath) throws ParseException {
		try {
			...
			writer.close();
			/*
			IndexSearcher countrySearcher = new IndexSearcher(DirectoryReader.open(FSDirectory.open(Paths.get(indexPath+"/countryIndex"))));
			String place="reunion";
			Query q = new QueryParser("country", analyzer).parse("\""+country.toLowerCase()+"\"");
			...
			if(hits.length>0) {
			    ...
			    System.out.println(doc.get("country")+"\t"+doc.get("ccode"));
			}*/
		} catch (IOException e) {
