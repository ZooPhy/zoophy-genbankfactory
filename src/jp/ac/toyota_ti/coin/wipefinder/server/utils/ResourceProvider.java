 ...
 * Properties Files, SockRoom... Usage: By default the resource provider can
 * gives access to the Database Interface, the default properties File and the
 * Taxonomy provider. Each resource has to be added to the provider, ex:
 * DBInterfacer dbi = new
 * DBInterfacer((String)pp.getValue("annotation.DB.Name"),
 * (String)pp.getValue("annotation.DB.Host"),
 * (String)pp.getValue("annotation.DB.User"),
 * (String)pp.getValue("annotation.DB.PW"));
 * Provider.addResource(RP_PROVIDED_RESOURCES.ANNOTATION_DBI, dbi); Or
 * WikipediaCategorizer Categorizer = new JWPLWikipediaCategorizer();
 * Provider.addResource("WipeFinderCategorizer", Categorizer); Then the
 * resources can be accessed as follow:
 * (String)ResourceProvider.getPropertiesProvider(RP_PROVIDED_RESOURCES.
 * PROPERTIES_PROVIDER).getValue("mysqlDBName"); Or
 * (WikipediaCategorizer)ResourceProvider.getPropertiesProvider(
 * "WipeFinderCategorizer");
 * 
 * @author dw
 *
 */
public class ResourceProvider {
	...
	/**
	 * The Enum types to the resources known and provided by default by the
	 * Resource Provider
	 * 
	 * @author dw
	 *
	...
	public static enum RP_PROVIDED_RESOURCES {
		ANNOTATION_DBI, // the default Interface to the DB which contain the
						// corpus annotated to work on
		PROPERTIES_PROVIDER, // the default properties files associated with the
								// current program
	...
	/**
	 * A common stock room (instantiate when it's retrieved for the first time
	 * to the ResourceProviser)
	 */

	/**
	 * Create a ResourceProvider with a default {@link #resourcesMap}, if
	 * another {@link ResourceProvider} exists, the previous resourceMap is
	 * used.
	 */
	...
	/**
	 * Search a {@link PropertiesProvider} to the default properties file i.e.
	 * PROPERTIES_PROVIDER
	 * 
	 * @param pPropertiesProvider
	 * @return the default properties, null if it doesn't exist or if something
	 *         else is asked
	 */
	...
	/**
	 * Add a new resource to the {@link ResourceProvider} If an existing
	 * resource exist nothing is done
	 * 
	 * @param resourceName
	...
	/**
	 * Add a new provided resources to the {@link ResourceProvider} If an new
	 * DBI is added while a DBI is already registered nothing is done (use
	 * {@link #removeResource} before) If an new PropertiesProvider is added
	 * while a PropertiesProvider is already registered we merge the values (if
	 * an existing properties have the same name, the first one is kept)
	 * 
	...
	/**
	 * Remove a resource from the provider, nothing done if the resource doesn't
	 * exist
	 * 
	 * @param pProvidedResource
	 *            - resource to remove
		...
		if (resourcesMap != null) {
			for (Object resources : resourcesMap.values()) {
				if (resources instanceof PropertiesProvider) {
					...
					log.info("Warning, I found a resource [" + resources.getClass().getSimpleName()
							+ "] which is not close properly.");
				}
			}
			// I close the PropertiesProvider at the end since I may need some
			// information inside to close the other resources
